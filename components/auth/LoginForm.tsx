"use client";
import { Eye, EyeOff, BookOpen, ShieldCheck, Sparkles, UserCircle } from "lucide-react";
...
interface LoginFormProps {
  roleName: string; // VD: "Học Viên", "Giáo Viên", "Admin"
  rolePath: string; // VD: "/student", "/teacher"
}

export function LoginForm({ roleName, rolePath }: LoginFormProps) {
  ...
  return (
    <GuestGuard redirectTo={rolePath}>
    <div className="flex min-h-screen bg-gray-50 text-gray-900">
      ...
      <div className="flex w-full items-center justify-center bg-gray-50 p-6 sm:p-12 md:p-16 lg:w-1/2 lg:p-24 min-[2400px]:p-28 relative">
        
        {/* Nút chuyển đổi portal tạm thời */}
        <div className="absolute top-6 right-6 group">
          <button className="flex items-center gap-2 text-xs font-extrabold text-gray-500 hover:text-primary transition-colors bg-white border border-gray-200 px-4 py-2 rounded-2xl shadow-sm">
            <UserCircle className="w-4 h-4" /> Portal Khác
          </button>
          <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-gray-100 rounded-2xl shadow-xl p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
            <Link href="/student/login" className="block px-4 py-2 text-xs font-bold text-gray-600 hover:bg-pink-50 hover:text-primary rounded-xl">Đăng nhập Học Viên</Link>
            <Link href="/teacher/login" className="block px-4 py-2 text-xs font-bold text-gray-600 hover:bg-pink-50 hover:text-primary rounded-xl">Đăng nhập Giáo Viên</Link>
            <Link href="/mentor/login" className="block px-4 py-2 text-xs font-bold text-gray-600 hover:bg-pink-50 hover:text-primary rounded-xl">Đăng nhập Mentor</Link>
            <Link href="/admin/login" className="block px-4 py-2 text-xs font-bold text-gray-600 hover:bg-pink-50 hover:text-primary rounded-xl">Quản Trị Viên</Link>
          </div>
        </div>
        ...
        <div className="auth-form-card w-full rounded-3xl border border-gray-100 bg-white p-8 shadow-soft sm:p-10 space-y-8">
          <div className="text-center md:text-left space-y-2">
            <h2 className="text-3xl font-black text-gray-950 tracking-tight">Đăng nhập {roleName}</h2>
            <p className="text-xs text-gray-500 font-medium">
              Vui lòng đăng nhập vào tài khoản St3pLearn của bạn để tiếp tục.
            </p>
          </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit(onSubmit, onInvalid)}>
            ...
            >
              {isLoading ? "Đang xử lý đăng nhập..." : `Đăng Nhập ${roleName}`}
            </button>
          </form>
