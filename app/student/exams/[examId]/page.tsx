...
export default function StudentExamPage({ params }: { params: Promise<{ examId: string }> }) {
  ...
  if (!attempt) return <div className="w-[500px] max-w-full mx-auto rounded-3xl border bg-white p-8 text-center shadow-soft"><h1 className="text-2xl font-black">Bắt đầu bài thi</h1><p className="my-3 text-sm text-gray-500">Exam ID: {examId}. Đồng hồ bắt đầu ngay khi server tạo attempt.</p><button onClick={start} className="mx-auto flex items-center gap-2 rounded-xl bg-primary px-5 py-3 font-bold text-white"><Play className="h-4 w-4" />Bắt đầu</button></div>;
  if (submitted) return <div className="w-[600px] max-w-full mx-auto rounded-3xl border bg-white p-8 text-center">{resultError && <p className="text-red-600">{resultError.message}</p>}{!result && <Loader2 className="mx-auto h-6 w-6 animate-spin" />}{result && <><h1 className="text-2xl font-black">Kết quả bài thi</h1><p className="my-4 text-4xl font-black text-primary">{result.score ?? "—"}</p><p className="font-bold">{result.passed ? "Đạt" : "Chưa đạt"} · {result.status}</p></>}</div>;
 
  return <div className="w-[768px] max-w-full mx-auto space-y-5"><div className="sticky top-2 z-10 flex items-center justify-between rounded-2xl border bg-white p-4 shadow"><h1 className="font-black">Bài thi</h1><span className="flex items-center gap-2 font-mono font-black text-red-600"><Clock className="h-4 w-4" />{Math.floor(secondsLeft / 60)}:{String(secondsLeft % 60).padStart(2, "0")}</span></div>{attempt.questions.map((q, index) => <article key={q.id} className="rounded-2xl border bg-white p-5"><p className="font-bold">{index + 1}. {q.content}</p><p className="mb-3 text-xs text-gray-500">{(q.type === "SINGLE_CHOICE" && q.metadata?.isTrueFalse) ? "TRUE_FALSE" : q.type} · {q.points} điểm</p>{(q.type === "MULTIPLE_CHOICE" || (q.type === "SINGLE_CHOICE" && q.metadata?.isTrueFalse)) ? (q.metadata?.options?.map((option) => <label key={option.id} className="mb-2 flex gap-2 rounded-xl border p-3 text-sm"><input type={q.type === "MULTIPLE_CHOICE" ? "checkbox" : "radio"} name={q.id} checked={answers[q.id]?.selectedOptionIds?.includes(option.id) ?? false} onChange={() => setAnswers((value) => { const current = value[q.id]?.selectedOptionIds ?? []; const selectedOptionIds = q.type === "MULTIPLE_CHOICE" ? (current.includes(option.id) ? current.filter((id) => id !== option.id) : [...current, option.id]) : [option.id]; return { ...value, [q.id]: { ...value[q.id], selectedOptionIds } }; })} />{option.text}</label>)) : (<textarea value={answers[q.id]?.textAnswer ?? ""} onChange={(e) => setAnswers((value) => ({ ...value, [q.id]: { ...value[q.id], textAnswer: e.target.value } }))} className="w-full rounded-xl border p-3 text-sm focus:outline-none focus:ring-1 focus:ring-primary/30" placeholder="Nhập câu trả lời viết tay của bạn..." />)}</article>)}<button disabled={submitting} onClick={submit} className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary p-3 font-bold text-white disabled:opacity-50">{submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}Nộp bài</button></div>;
}
