...
export default function NewCoursePage() {
  ...
  const onSubmit = async (data: CourseCreateFormValues) => {
    ...
    try {
      // Gửi yêu cầu POST lưu khóa học mới lên API Gateway
      ...
      if (!res.ok) {
        throw new Error(body?.message || "Tạo khóa học thất bại.");
      }
      ...
      router.push(createdCourseId ? `/dashboard/teacher/courses/${createdCourseId}/curriculum` : "/dashboard/teacher");
    } catch (err: unknown) {
              ...
              >
                {isSaving ? (
                  ...
                  <>
                    ...
                    <span>Lưu khóa học & Tiếp tục</span>
                  </>
                )}
              </button>
            </div>
